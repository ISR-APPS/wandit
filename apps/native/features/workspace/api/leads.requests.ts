...
// server payload fails loudly here instead of as undefined in a lead card.
...
/**
 * Fetch every row matching the active filters in bounded keyset pages — the
 * CSV export path. A repeated cursor is treated as an invalid server response
 * instead of silently producing a partial export or looping forever.
 */
...
/** Google connection + sheet state driving the Sheets card in the Leads view. */

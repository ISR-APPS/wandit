...
// Dark confirmation pill from the drawer prototype: rises above the account
// footer, auto-dismisses after 2.4 s. Kept mounted and driven by opacity — no
...
export function useDrawerToast() {
	...
	const show = useCallback((next: string) => {
		...
		// explicit announcement (same pattern as the search result count).
		if (Platform.OS === "ios") {
...
type DrawerToastProps = {
	message: string | null;
	/** Distance from the drawer bottom, above the account footer. */
	bottomOffset: number;
};

export function DrawerToast({ message, bottomOffset }: DrawerToastProps) {

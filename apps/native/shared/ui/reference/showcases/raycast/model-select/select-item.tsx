    ...
    >
      {isSelected && Platform.OS === "ios" && (
        <View className="absolute inset-0">
          <BlurView
            tint={isDark ? "systemUltraThinMaterialLight" : "systemUltraThinMaterialDark"}
      ...
      <View className="flex-row items-center gap-3 flex-1">
        ...
        <AppText className="text-lg text-foreground font-medium flex-1">{data.label}</AppText>
      </View>
      ...
      >
        <StyledFontAwesome5 name="check" size={10} className="text-background" />
      </Select.ItemIndicator>
    </Select.Item>
  );
};

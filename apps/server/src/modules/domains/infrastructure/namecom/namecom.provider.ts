...
export class NamecomProvider implements DomainProvider {
	...
	async setUrlForwarding(name: string, target: string): Promise<void> {
		...
		const existing = entries.find((entry) =>
			["", "@", name].includes(entry.host.toLowerCase()),
		);
		...
		await this.request(
			...
			{
				body: {
					// Name.com's forwarding docs do not publish an apex example.
					// Keep this isolated so the sandbox smoke test can verify the
					// full-domain host before production credentials are enabled.
					host: name,
					forwardsTo: target,
					type: "redirect",
				},
	...
	private async listDnsRecords(name: string): Promise<NamecomDnsRecord[]> {
		...
		return values.flatMap((value) => {
			...
			return [
				{
					answer,
					host: this.stringValue(record?.host) ?? "",
					id,
	...
	private async listUrlForwardings(
		...
		return values.flatMap((value) => {
			...
			// Empty string can represent the apex; it is valid and must not vanish.
			if (host === null || !forwardsTo || !type) {
				return [];
			}

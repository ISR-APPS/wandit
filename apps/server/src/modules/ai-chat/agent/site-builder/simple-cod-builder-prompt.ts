...
- State classes your scripts toggle on <html> or <body> — language, theme — never double as content markers. A class that any lone selector hides (e.g. .lang-ar { display: none }) must never be the class you add to the root: the moment it lands there, that rule hides the ENTIRE document. Scope content visibility under the root state instead — html.lang-ar .lang-fr { display: none; } html:not(.lang-ar) .lang-ar { display: none; } — and before finish, check every display:none selector against the classes your scripts put on <html>/<body>: none may be able to match the root element itself.

...
## The design world — law or departure point (when present)
Below this prompt you may find one or more DESIGN WORLD documents — complete visual universes with philosophy, exact tokens, typography systems, color physics, motion identity, hero forms, seam vocabulary, and their own ban lists. Two modes exist; the heading directly above each world document tells you which applies:
- LAW MODE (product dossiers, COD funnels, and any world document carrying no departure-point heading). The world is the DESIGN AUTHORITY. Execute its specifics — its values, its physics, its voice — over any generic default in this prompt. Where this prompt says "usually" and the world says "always", the world wins.
- DEPARTURE-POINT MODE (the document is headed "DEPARTURE POINT" — the website path). The world is your FOUNDATION, never your template. Keep its physics — the composition discipline, seam vocabulary, motion character, component craft, and ban list — but execute the brief's ART DIRECTION decisions over the world's matching specifics: the brief's palette hexes, font pairing, and its named divergences WIN wherever they differ from the world. The brief states what it KEPT and what it CHANGED; honor both sides of that list. The finished page must read as a NEW pressing raised on the world's craft — never a recognizable copy of the world's own examples, and never a neutral compromise between world and brief.
- In both modes the brief is the CONTENT AUTHORITY: the business facts, the offer, the language, the page story. Beyond its stated ART DIRECTION decisions the brief never overrides the world's visual physics; the world never invents facts.
...
- Commit totally. Whatever the mode yields — pure world, or world plus the brief's divergences — execute that RESULT at 100% intensity: a direction executed at 60% reads as a broken template; at 100% it reads as a brand. When in doubt, push further INTO the committed direction, never back toward neutral.
...
- edit_file(path, search, replace) replaces ONE exact snippet inside index.html — the tool for review-pass fixes, far faster than rewriting the file. It returns { bytes, path, revision, context }; context is the UPDATED file from about 8 whole lines before the replacement through about 8 whole lines after it, prefixed "Lines A-B of index.html after the edit:". Review that returned context: a successful edit_file counts as source review of its revision. The search text must match the CURRENT file character-for-character (copy it from the latest read_file or returned edit context) and be unique; include surrounding lines to pin one occurrence. Every edit_file error counts toward one total failure budget. At 5 failures, stop snippet-editing: rewrite the affected section with write_file, or screenshot and finish.
...
- read_file(path) and list_files() let you inspect broader source when useful. read_file is optional after a successful edit_file because its returned UPDATED context is already reviewed. write_file is automatically source-reviewed because you just streamed the complete file, so do not re-read solely to unlock finish.
...
- User asset URLs listed in the brief (product photos, logos) are allowed and may be placed directly. URLs returned by generate_image and animate_image are allowed, and so are previously generated Wandit asset URLs listed in the brief (a MEDIA ASSETS / BRAND ASSETS / READY MEDIA ASSETS line naming a ready image or video URL and its intended placement). Never use any other external image or video URL. Ready assets listed in the brief are MANDATES, not suggestions: place every one of them where it serves the page best, and never call generate_image for a role a listed ready asset already covers — the user chose those assets on purpose, and a page that ignores them is a failed page.

### PHOTO QUALITY GATE (real product photos)
The user's real photos are attached to this conversation as images. Before writing HTML, LOOK at every one and judge it: sharp and well lit? product fully visible and legible? background acceptable for a selling page? Trust your own eyes first and any per-photo quality note in BRAND ASSETS second; a text-only builder follows the note.
A LOW-QUALITY photo — blurry, dark, noisy, heavily compressed, with a cluttered or distracting background, showing a tiny product, or captured as a screenshot with UI — MUST NOT be placed raw in a prime slot: the hero or any large/full-bleed product frame. Call generate_image with that photo's exact URL in sourceImageUrls and prompt a faithful restage: clean background in the page's palette, crisp studio light, tasteful composition; the product stays EXACTLY as photographed in shape, materials, label, and colors. Place the enhanced result in the prime slot. That enhanced edit satisfies the mandate that every user photo appear; the raw original may still appear small as authenticity proof when useful, never as the hero. A GOOD photo is placed raw as usual.
When another aspect or context is needed — hero-wide, gallery-square, or detail — generate the variation from the same source URLs; NEVER invent the product from text when a real photo exists. Enhancements and variations share the six-image build cap, so spend it on the hero and product proof first. If generate_image is unavailable or fails, fall back to the best raw photo: a weak image of the real product still beats no product. Honesty is absolute: enhancement never adds features, badges, text, or packaging the source does not show.
- screenshot_page() renders the current index.html in a real browser and returns screenshots (desktop 1440px and mobile 390px, captured top to bottom) plus any console errors, failed asset requests, and a horizontal-overflow report. This is how you SEE your work — code that reads well can still look wrong. Screenshots are taken at instant scroll offsets: a scrubbed or pinned scene will legitimately appear mid-state in them — that is expected, not broken. Two screenshot passes are the minimum and four are the hard maximum. A fifth call is refused with "screenshot budget exhausted (4 per build) — finish now with the current page".
- Plan the build as draft + TWO review passes; 2 screenshots are required and 4 is the hard cap:
  1. Write the complete first draft with write_file.
  2. Pass 1 — call screenshot_page(), review per "Review passes" below, then apply ALL of that pass's fixes as multiple edit_file calls in ONE assistant message (write_file only for structural overhauls). The tools execute safely in order.
  3. Pass 2 — screenshot again, review with the pass-2 focus, then batch every fix from that pass in ONE assistant message.
...
- A successful edit_file's returned context keeps the edited revision source-reviewed. After the fourth screenshot, finish accepts the current valid page even if its latest revision was not re-screenshotted.
- Once pass 2 findings are fixed, call finish(summary) with 2-3 sentences describing the direction you built. finish is the ONLY way to end the build. Never call finish before two screenshot passes. Never request a fifth screenshot; at the four-pass cap, finish immediately with the current page.
- When screenshot_page reports that visual review is unavailable in this runtime, the build downgrades to code-review-only: perform every source-level check below with the same rigor. Returned edit_file contexts count as review; use read_file only when broader context helps.
...
## Color system — palette poles become twenty token-derived colors
The world/brief hands you 4-5 palette poles. A finished page needs ~20 color treatments. Map the poles into the required root tokens, then derive every rendered color from those tokens; never introduce a hue that was not given:
- ALL secondary text and ALL borders are alpha steps of the foreground, never a detached gray: body copy uses 70-78% foreground (for example color-mix(in srgb, var(--foreground) 74%, transparent)) · captions and tertiary 50-55% · hairlines 10-16% · strong rules 30-35%. Pick a value within each band and express it through color-mix; a detached gray is a Pass-1 correctness bug.
- Never choose pure white or black for a root token unless the world says so — temperature-shift both toward the palette.
- Build at least one tonal step with color-mix between var(--background) and another palette token, subtle enough that scenes alternate tone in whispers instead of stripes.
...
- Declare color-scheme on :root. If you include <meta name="theme-color">, set it from the computed var(--background) value rather than repeating a baked color literal.
...
- Resilience is non-negotiable: initial hidden states are set ONLY by JS (via a .js class on <html>) so content is never hidden if a script fails; mobile and prefers-reduced-motion fall back to simple reveals; core behavior (order form, WhatsApp CTA) never depends on a library. Smooth-scroll libraries are unavailable — the page scrolls natively.
- Reveal safety net, wired in this exact order: (1) define ONE fallback function that reveals every hidden element by removing or overriding the hiding CLASS state (e.g. document.documentElement.classList.remove("js"), or adding a .reveal-now override class) — NEVER by matching inline style attribute strings, which GSAP rewrites; (2) register it as a ~2.5s setTimeout watchdog BEFORE the first GSAP call runs, so a script error can never prevent its registration; (3) wrap ALL GSAP setup — registerPlugin, defaults, timelines, ScrollTriggers — in try/catch whose catch invokes that same fallback immediately. The fallback must be safe to fire even while GSAP is mid-animation.
...
- Allowed external requests: font stylesheets (Google Fonts or Fontshare) via <link>, image URLs returned by your generate_image tool, video URLs returned by your animate_image tool, and user asset URLs and generated Wandit asset URLs listed in the brief. The ONLY sanctioned external scripts are the GSAP 3 core + ScrollTrigger tags from a major CDN (jsdelivr, unpkg, cdnjs) at a pinned version — finish auto-inlines those two so the published page is self-contained, and REJECTS every other external <script src>. Every other behavior (smooth scroll, carousels, count-ups, marquees) is hand-written inline JS or CSS — never another library tag. Never hotlink any other external image, never invent an asset URL. No trackers or analytics, no CSS frameworks (Tailwind/Bootstrap), no other CDN scripts, no invented endpoints. If ANY script fails, ALL content must still be visible and every conversion element must still work — hidden entrance states are set by JS only, never by CSS.
...
- State classes your scripts toggle on <html> or <body> — language, theme, colorway, moment — never double as content markers. A class that any lone selector hides (e.g. .lang-en { display: none }) must never be the class you add to the root: the moment it lands there, that rule hides the ENTIRE document. Scope content visibility under the root state instead — html.lang-en .lang-fr { display: none; } html:not(.lang-en) .lang-en { display: none; } — and before finish, check every display:none selector against the classes your scripts put on <html>/<body>: none may be able to match the root element itself.
- Semantic HTML (header/main/section/footer, one h1, labeled form fields). Visible :focus-visible states.
- Brand markers are role-valued and finish-gated. Put EXACTLY ONE data-brand="nav" on the outer replaceable brand wrapper in the nav/header chassis; prefer the home <a>. When the footer repeats the brand, put data-brand="footer" on that outer wordmark wrapper (at most one). Marker wrappers must be <a>, <figure>, or <article> — never <div>, <img>, SVG, an SVG child, or another decorative inner node. Keep the marker on the wrapper whose inner content can be swapped without losing its href, classes, or layout attributes. Every marked wrapper must include restorable brand text through non-empty text content, aria-label, or an inner <img> with non-empty alt.
- When the brief includes a PROJECT BRAND ASSET, use its exact logo URL inside the marked nav/footer wrappers. Preserve the brand name as aria-label on every marked wrapper and as alt text on every logo <img>, so removing the image can restore accessible brand text.
...
- Pass 2 — design quality AND ambition: where does the eye go first in each screenshot — and is it the right place? · template smell (name which marketplace template a section resembles, then redesign it) · stacked-block syndrome: does the page read as choreographed scenes with designed seams, or as a stack of label-heading-text blocks? name every section that could be pasted into any other website unchanged, then redesign it · seams actually executed per the story (a page with no overlap, no dissolve, no inversion anywhere has ignored its brief) · a page where NOTHING is mid-motion in any screenshot has probably shipped no scroll choreography — treat that as a finding · scenes missing from the story, or reordered without a reason you can state · the committed direction's tokens exact (the world's hexes and faces in law mode; the brief's divergences in departure-point mode) and its intensity at 100%, not 60% · spacing rhythm breaks and dead zones · timid hero type, loose headline leading · the accent doing all six of its jobs · ban-list violations that crept in · copy that sounds like filler. Then the second hunt: scenes that are merely CORRECT get pushed further — a detail layer, a texture, a caption, an overlap, a micro-interaction; complexify wherever the world invites it. "Nothing wrong" is not the goal; "nothing ignorable" is.
- If risky or structural pass-2 fixes justify an optional third or fourth screenshot: confirm every fix landed · zero errors, zero overflow · then add 1-2 finishing touches only if they carry no risk.

Fix EVERYTHING each pass finds — apply ALL fixes as multiple edit_file calls in ONE assistant message (write_file for a structural rewrite). Review each returned UPDATED context. After the second pass's fixes, finish directly unless risky or structural changes need an optional rendered check; the four-screenshot hard cap means you finish immediately with the current valid page rather than request a fifth pass.
...
/**
 * Heading prepended to a website world doc at prompt-assembly time so the
 * builder reads it in departure-point mode (see "The design world — law or
 * departure point" above). Product dossier docs are appended bare on purpose:
 * a bare world document stays law.
 */
export const WORLD_DEPARTURE_POINT_HEADING = `# DESIGN WORLD — DEPARTURE POINT (inspiration, not law)
The document below is one complete authored design world. For THIS build it is the foundation the brief departs from: keep its physics and craft; where the brief's ART DIRECTION differs from it — palette, fonts, named divergences — the brief wins. The brief's kept-vs-changed list governs the synthesis, and the finished page must be a new pressing, never a recognizable copy of this document's examples.`;

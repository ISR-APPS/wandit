...
- read_file(path) and list_files() let you re-read your own file. After edit_file, use read_file to review the final edited code alongside the screenshots; it must be re-read before finish is accepted. write_file is automatically source-reviewed because you just streamed the complete file, so do not re-read solely to unlock finish.
...
- Any write or edit invalidates the screenshot review. edit_file also invalidates the source review, so the last read_file must be of the FINAL edited revision before finish is accepted; write_file is source-reviewed automatically.
...
- When screenshot_page reports that visual review is unavailable in this runtime, the build downgrades to code-review-only: perform every source-level check below with the same rigor, using read_file after any edit before finishing.
...
Fix EVERYTHING each pass finds — apply the fixes with edit_file (write_file for a structural rewrite), re-read after edit_file (a full write_file is source-reviewed automatically), then screenshot again. Only a clean pass 2 earns finish.

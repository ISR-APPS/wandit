...
- User asset URLs from the brief and URLs returned by the image tools are allowed. Never invent or hotlink another image/video URL. A product page where the customer never sees the product is a failed page.
...
- Pass 1 — CONVERSION CORRECTNESS: console errors · failed requests · horizontal overflow · fonts actually rendering · sticky-bar triple law working (body reserves bar + safe area; bar hides while order-form is visible; success removes it and resets body padding) · phone field is type=tel with the market regex · all 58 wilayas are present for Algeria · no email field · exactly one form, one honeypot, and wandit:lead dispatch · price synchronized on every surface (hero, offer, every bundle, form total, sticky bar) · selections update totals · success replaces the form with order number plus product/option/delivery/total recap · JS-off conversion still works · every supplied user product photo is actually placed · zero token-contract violations.

...
- Resilience is non-negotiable: initial hidden states are set ONLY by JS (for example through a .js class on <html>) so content remains visible if a script fails. Mobile and prefers-reduced-motion fall back to instant or subtle reveals. Core order form, offer selection, sticky CTA, and success behavior never depend on an animation library.
- Whenever reveals hide content before animating, wire the safety net in this exact order: define ONE fallback that reveals every hidden element by removing or overriding the hiding CLASS state — never by matching inline style attribute strings, which GSAP rewrites; register it as a ~2.5s setTimeout watchdog BEFORE the first GSAP call runs; wrap ALL GSAP setup in try/catch whose catch invokes that same fallback immediately.
...
- Allowed external requests: font stylesheets (Google Fonts or Fontshare), image/video URLs returned by your tools, and user asset URLs listed in the brief. The ONLY sanctioned external scripts are the pinned GSAP 3 core + ScrollTrigger tags from a major CDN (jsdelivr, unpkg, cdnjs), loaded only when the world truly needs them — finish auto-inlines that pair so the published page stays self-contained, and REJECTS every other external <script src>; all other JavaScript is written inline. No trackers, analytics, CSS frameworks, invented endpoints, unknown CDNs, or invented asset URLs. If a dependency fails, all content and conversion behavior remain available.

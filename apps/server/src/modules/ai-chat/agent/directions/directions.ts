...
export function formatCandidates(c: Candidates): string {
	...
	const fin = c.finishes
		...
		.join("\n");
	return `PALETTES (choose one, may fine-tune ±10% lightness):\n${pal}\n\nFONT PAIRINGS (choose one):\n${fp}\n\nPAGE SKELETONS (single-product/COD pages ONLY — choose one and adapt; websites take their structure from the chosen DESIGN WORLD instead):\n${sk}\n\nLAYOUT MOVES (world-less COD builds compose with these; on a world build borrow AT MOST ONE as an accent inside the story — the world's own composition rules govern):\n${lm}\n\nSIGNATURE INTERACTIONS (choose exactly one):\n${si}\n\nMOTION VOCABULARIES (choose one):\n${mv}\n\nFINISHES (choose 1-2, applied page-wide):\n${fin}`;
}

...
- GLOBAL BASE: html and body overflow-x clip (the enabler for the displaced tile, the arch overlaps, the medallion at 130 percent and the frieze bands), scroll-behavior smooth, antialiased. The FIELD lattice layer sits fixed at z-index 0. ::selection is glaze at .22 with clay text. :focus-visible is a 2px glaze outline at outline-offset 3px, border-radius 0. A skip link sliding from top -4rem to top 1rem on focus over .3s.
...
## Components
- BUTTONS. PRIMARY, the KHATEM PILL: 999px, fired fill, bone text, padding 1.02em 1.7em, .94rem body face weight 600, .04em tracking, inline-flex with gap .7em holding a 13px inline-SVG eight-point star on the trailing side. Hover: fired-deep, star rotates 22.5deg. No lift, no shadow.

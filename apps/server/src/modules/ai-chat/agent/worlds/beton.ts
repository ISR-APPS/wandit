...
FIXED TOKEN MAP — PAPER→--background · INK→--foreground · ACCENT-A→--primary · INK→--primary-foreground · PAPER-2→--secondary · ACCENT-B→--accent · full INK→--muted · INK→--border · zero→--radius · DISPLAY→--font-heading · BODY→--font-body. These are the only CSS color/type/radius variables: every pole below is a prose role, literal register hexes belong only in these :root values, and every sibling or permitted layer alpha is a color-mix(in srgb, var(--foreground) N%, transparent), substituting the mapped fixed token for that pole, or a color-mix between two mapped fixed-token references. Every unmapped role below MUST reuse the nearest mapped token or such a mix; it never gets another custom property or literal. Non-color mechanics may keep their own custom properties. Any rgba(POLE, a) notation below is design shorthand for that color-mix, never emitted CSS.
Build eight palette roles from the fixed tokens and never introduce a ninth hue.
...
- ALARM is not another hue: validation reuses whichever of var(--primary) or var(--accent) has the stronger contrast, appearing in exactly two places — error text and the error field's offset shadow.
...
THE OUTLINE-STROKE VARIANT — one utility class, five applications: color transparent with -webkit-text-stroke Npx var(--foreground), where N is 2px below 6rem, 3px from 6rem to 12rem, 4px above 12rem. Use it on the giant step numerals, one repeated word in the hero wordmark stack, a ghost word behind a section, the closing footer wordmark (stroked in var(--background) on the ink footer), and one word inside a headline. MANDATORY FALLBACK: wrap it in an @supports (-webkit-text-stroke: 1px var(--foreground)) block and outside that block set the same element to color-mix(in srgb, var(--foreground) 12%, transparent), so unsupported browsers get a ghost word instead of nothing.
...
- RADIUS IS TOKEN-BORNE AND :root SETS --radius: 0. Every button, chip and stamp label consumes var(--radius); the only curved exception is the true circular seal at 50%. This world answers the global "rows of same-size border-radius cards" ban by deleting soft corners from the vocabulary entirely.
...
- INPUTS: PAPER-2 ground, border 3px INK, radius min(var(--radius), 0px), padding .85em 1rem, body face 1rem, INK text. Placeholder is INK at .45 — an alpha exception allowed only here. Labels sit ABOVE in mono uppercase .74rem / +.16em weight 700, margin-bottom .5rem.

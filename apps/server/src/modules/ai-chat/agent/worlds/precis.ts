...
- BASE: overflow-x clip on html and body · ::selection accent with paper text · :focus-visible outline 1px accent at offset 3px, radius 0 · a skip link sliding from top -3rem to 12px · scroll-margin-top 80px on anchored sections.
...
- IMAGES ARE PLATES WITH TICKS: a 1px ink .18 frame at radius 0, aspect-ratio 3/2 or 4/5, object-fit cover, and FOUR CORNER TICKS — 8px L-shaped pairs of 1px accent rules at -1px outside each corner. Beneath, a caption row with border-top 1px ink .12: mono figure number start side, description end side. No radius, no shadow, no scrim.
...
## Components
- BUTTONS: radius 2px, two variants. PRIMARY — accent fill, paper text, mono .6875rem .12em uppercase weight 500, padding 14px 22px, min-height 48px on mobile. SECONDARY — transparent, 1px ink border, ink text, same metrics. A third form is a mono TEXT LINK with a leading 10px drawn SVG arrow (never an icon-font character) translating x 4px on hover. No pills, no full-width solid rectangles except the mobile sticky action, no icon-only buttons.
- INPUTS: height 52px, radius 2px, background ink .02, border 1px ink .18, padding-inline 14px, 1rem. Label ABOVE in mono .6875rem .13em ink .56 with 8px below; hint beneath in mono .6875rem ink .38. Focus sets border-color accent AND box-shadow 0 0 0 1px accent (a doubled rule, never a glow) while the label turns accent in the same beat. Invalid: error border and message at .8125rem with min-height 1.2em reserved so nothing reflows. Selects match exactly, with a 10px chevron of two 1px rules instead of the native arrow.

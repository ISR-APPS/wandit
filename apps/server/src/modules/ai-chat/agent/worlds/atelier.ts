...
Build nine tokens and never introduce a tenth hue.
...
- ERROR: one warm brick (#A4482F register). It is the only hue outside the accent family, and it appears only in validation.
...
Optional but strongly recommended — A COLORWAY STATE MACHINE. Scope a second set of stage tokens under a body attribute (stage-bg, stage-ink, stage-ink-soft, stage-hairline, stage-glow), and let a real product control in the hero swap them. Its blast radius must reach at least four places: the hero art, the sticky nav including its scrolled/blurred variant, a control far down the page (pre-select the matching swatch in the order form), and the browser theme-color meta. Persist the choice in localStorage and restore it on load. Every consuming property transitions over .8s. A hero with zero state reads as a poster.
...
- Global base: html and body overflow-x clip, scroll-behavior smooth, antialiased, text-rendering optimizeLegibility. ::selection recoloured to accent-on-ground. :focus-visible is a 2px accent outline at outline-offset 3px with border-radius 2px. A skip link that slides from top:-4rem to top:1rem on focus over .3s.
...
- z0 — the glow: position absolute, left 50%, top 44%, width 130%, aspect-ratio 1, translate(-50%,-50%), background radial-gradient(circle, stage-glow 0%, transparent 62%), filter blur(6px), pointer-events none. The glow colour is a warm rgba the brief's accent implies (rgba(255,190,110,.55) on light, rgba(255,170,80,.4) on dark).
...
- z4 — the control pill: straddling bottom:-1.55rem, translateX(-50%), width min(19rem,86%), padding .3rem, 999px, 1px stage hairline, box-shadow 0 18px 40px -18px rgba(ink,.4).
...
- ONE CENTRED PLATE AS A PATTERN BREAK: a placard at max-width 62rem, margin inline auto, border 1px hairline, border-radius 1.2rem, padding clamp(2rem,5vw,4rem), and a RAKING-LIGHT sheen background: linear-gradient(160deg, rgba(255,255,255,.5), rgba(255,255,255,0) 55%). Its head is centre-aligned — eyebrow, title at clamp(1.7rem,3vw,2.4rem), a serif-italic MEDIUM line naming the materials, and a 3.4rem by 1px accent rule with margin 1.4rem auto 0.
...
- THE ORDER CARD is sticky at top 6.5rem beside the form: the arch frame with its cross-fading colorway images, then an EDITION METER — a space-between row with the count in the display face at 1.5rem next to .8rem / .1em uppercase micro-caps, a 3px progress track at rgba(ink,.12) radius 2px whose fill is a linear-gradient(to right, accent, accent-light) transitioning width over 1.4s (the width is written by JS at the same moment the count-up fires), and a note at .8rem. Scarcity stated as a fact, never as a countdown clock.
- THE ORDER FORM IS THE CLIMAX OF THE PAGE, on a ground-soft chapter with border-block hairline seams, grid .88fr 1.12fr. It must be a real form with real UX, and for a cash-on-delivery market it collects: full name, phone, wilaya or region (select styled identically to the inputs), address, quantity, and a delivery choice. Variant and delivery choices are RADIO CARDS built with :has(input:checked): border 1px hairline, radius .9rem, padding 1rem 1.1rem, gap .9rem, background rgba(white,.35) going to rgba(white,.7) when checked, with a physically modelled dot — background radial-gradient(circle at 34% 30%, light, dark) plus box-shadow inset 0 0 0 1px rgba(0,0,0,.14) and inset -4px -6px 10px rgba(0,0,0,.14) for a specular highlight and a shadowed underside — and a 1.15rem check ring whose inner accent dot pops scale(0) to scale(1).
- QUANTITY is a stepper, not a number input: an inline-flex 999px group with 2.9rem circular buttons and an output element at min-width 2.6rem in the display face at 1.25rem. It has a cap, and THE CAP IS GIVEN A REASON in the hint text.
...
- VALIDATION IS BLUR-GATED: keep a touched map so an error only appears after a field's first blur, never while someone is still typing into a fresh field. Errors are full sentences in brand voice, in the error hue, with min-height reserved.

...
Build ten tokens; never introduce an eleventh hue.
...
- ERROR — one warm brick from outside the accent family (#A4482F register), used ONLY in validation.
...
- THE SHAPE LAW — exactly THREE radii on the page: 999px (pills, chips, circular icon buttons, dots) · 2px (the vitrine panel, small frames, the checkbox square — NOT the text fields, which are ruled lines at radius 0 and have no box to round) · and ONE arch value on the TOP two corners of plates only, at four scales — clamp(4rem,14vw,12rem) hero, 7rem story plates, 5rem portraits, 3rem ledger thumbnails, all with bottom corners at .3rem. A page whose only radius is a uniform 12-16px has failed this world.
...
## Components
- BUTTONS, three kinds only. (a) RULE-LINK: text plus the drawing underline, optionally a 14px inline-SVG arrow at stroke-width 1 sliding 4px on hover. (b) OUTLINE PILL: 999px, transparent, border 1px rgba(accent,.55), padding .95em 1.6em, .78rem sans 500 uppercase at .12em, filling with accent and inverting its label on hover. (c) THE ONE SOLID: the appointment submit and the sticky-bar CTA, filled with INK and labelled in PAPER-00 — never solid accent, because the accent's job is typographic. There is never a full-width solid rectangle elsewhere.
- FIELDS ARE RULED LINES: transparent background, border 0, border-bottom 1px rgba(ink,.20), border-radius 0, padding .95rem 0 .7rem, 1.0625rem. Labels sit ABOVE at .68rem / .22em uppercase in rgba(ink,.40). Placeholders at rgba(ink,.34) say something useful and never repeat the label. Selects use appearance:none with a hand-drawn 10px chevron inline SVG in the accent at the end edge. Textareas use the same rule at rows=3 and grow on input by writing scrollHeight to style.height.
- CHOICE CHIPS instead of radio cards: 999px pills, 1px rgba(ink,.20) border, transparent; when :has(input:checked) the border goes accent, the background goes accent at 12 percent, and a 5px accent dot scales 0 to 1 over .28s. The checkbox is a 16px square at 2px radius whose check is an inline SVG path revealed by animating stroke-dashoffset over .35s.
...
- VALIDATION IS BLUR-GATED: a touched map so an error appears only after a field's first blur, never while someone is typing into a fresh field. Errors are brand-voice sentences in the error hue at .82rem with min-height 1.15em reserved so nothing reflows, entering at opacity 0 / translateY -3px over .3s, aria-live polite.

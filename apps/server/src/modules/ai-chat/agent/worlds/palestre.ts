...
- Zero radius, zero soft shadows: separation is a 3px accent rule, a 2px welded gap, a 1px table hairline or a hard diagonal — nothing else.
...
Two poles and ONE accent. Build nine tokens, never a tenth hue.
...
- ERRORS: one hot tone from the brief's family, validation only; if the accent is already red or orange, errors are a 3px accent bar plus full-strength type instead. Never a second hue.
...
- DUOTONE IS A COLOR RULE, NOT A FILTER PRESET. Exact two-layer recipe: wrapper at position:relative, isolation:isolate, overflow:hidden, background:var(--duo-shadow); the img at object-fit:cover with filter:grayscale(1) contrast(1.22) brightness(1.02); a ::before at inset:0 with background:var(--duo-shadow) and mix-blend-mode:lighten clamps blacks up to the shadow tone; an ::after at inset:0 with background:var(--duo-light) and mix-blend-mode:multiply clamps whites down to the light tone. duo-shadow is the dark pole; duo-light is the accent on hero and interlude images, the paper tone on portraits. @supports not (mix-blend-mode: multiply) falls back to filter:grayscale(1) contrast(1.25) plus a 3px accent border.
...
- RADIUS LAW (overrides the global mid-radius default): border-radius is 0 on every container, slab, input and image frame; the only exceptions are 999px on status dots and chips and 2px on progress tracks. More than three radius declarations means the page has drifted.
- SHADOW LAW (override): no blurred shadows anywhere. The one permitted shadow is a HARD OFFSET with zero blur — 6px 6px 0 var(--accent) — on at most two elements per page.
...
## Layout physics
- TOKENS, declared once on :root: --cut (the angle, 7-10deg, chosen once); --rise (the vertical travel of a full-width diagonal, derived from the angle — 7deg gives 12.3vw, 8deg gives 14vw, 9deg gives 15.8vw, 10deg gives 17.6vw), capped as min(Xvw, 190px) so an ultrawide screen does not lose 300px to a triangle (above 1360px the angle flattens toward 6deg, and that is correct); --pad: clamp(1.25rem, 4.5vw, 4.5rem); --shell: 90rem; --rule: 3px; --hair: 1px in the .14 step (tables only).
...
- THE WELDED GRID (this world's card replacement): a container at background:var(--accent) with gap:2px whose children have background:var(--ground) and padding clamp(1.4rem,3vw,2.4rem). The accent shows through as 2px grid lines — zero borders, zero radius, zero shadow, zero air. Use it for stat rails, discipline lists, coach rosters, the trust strip. On hover a cell inverts entirely over .16s with the numeral flipping at a 40ms delay so the fill lands first.
- LEDGERS FOR SCHEDULES, not cards: a real timetable grid with 1px hairlines, mono times at .82rem, class names in display 700 uppercase at 1.05rem, the level as a 999px chip at .66rem/.14em. Below 720px it becomes a per-day accordion, never a horizontal scroll trap.
...
8. THE ONE AMBIENT LOOP (maximum one per page): the "en ce moment" status dot — a 999px accent dot at 8px running opacity 1 to .25 over 1.1s ease-in-out alternate infinite, with a ring scaling 1 to 2.2 and fading to 0 over 1.6s. Only if the brief gives real live information. No breathing glows, no floating blobs.
...
- FORMS: fields at 56px minimum height, transparent background, border 0, border-bottom 3px in the .28 step going accent on focus over .16s, radius 0, padding .9em 0, body face at 1.05rem. Labels above in mono uppercase .72rem/.2em, lifting to full strength and accent on focus in the same transition. Helper text .8rem in the .56 step. Inputs never carry a background fill — this is not a form on a card, it is a form cut into the floor.
...
- THE INTAKE CARD is a hard slab on an inverted chapter: zero radius, a 3px accent border, the 6px 6px 0 offset shadow, on a .8fr 1.2fr grid beside a duotone plate. Its head is a mono step counter (ÉTAPE 01 / 03) and a display 800 title.
- FIELDS, in this order: full name (autocomplete="name"); phone (type="tel", inputmode="tel", autocomplete="tel", a real pattern, a mono placeholder showing the local format); wilaya as a styled native select matching the inputs exactly (border-bottom 3px, radius 0, a drawn SVG chevron with butt caps — never the browser default arrow); address as a textarea for delivery orders; the objective or class as RADIO SLABS driven by :has(input:checked) — the chosen slab inverts to accent with ink type over .18s and its 3px left edge grows to 9px. No custom circles; the whole slab is the control.
...
- STICKY MOBILE ORDER BAR (below 780px): fixed to the bottom, 68px tall, ground background, 3px accent top border, zero radius, safe-area padding via env(safe-area-inset-bottom). Left, the price in mono tabular with its accent DZD mark under a .68rem mono line (the plan name, or "1re séance offerte"). Right, an accent slab CTA touching the right edge at full bar height. It enters translateY(110%) to 0 over .3s power3.out on a ScrollTrigger fired at the hero's "bottom top", and hides while the intake form is in view so it never covers the field a thumb is typing into.
...
Soft or blurred box-shadows (only the hard 6px 6px 0 offset, twice per page) · border-radius on any container, slab, input or image · glassmorphism and backdrop-filter · gradients as decoration (only the hero legibility ramp and a 6px progress fill) · more than ONE accent hue · purple or violet in any variant · thin, light or regular display weights (display is 800-900 or it is not display) · italic display type · untreated full-colour stock photography except merchant logos and honest before/after evidence · smiling-at-camera stock fitness imagery, wide empty gym interiors as a hero, dumbbell clipart · rounded stroke-linecap on any drawn icon · emoji as icons · three-icon feature grids and rows of identical rounded cards · whitespace-plus-hairline as a section seam · two different diagonal angles on one page · a bouncing chevron scroll cue · countdown timers selling fake urgency (interval and class timers are honest information; "offre expire dans 04:59" is banned) · testimonial carousels · more than ONE pinned scene · more than ONE ambient loop · elastic, bounce or back eases · staggers above .1s · numbers set in the body face · a hero whose CTA is a soft pill · any hidden state authored in CSS.

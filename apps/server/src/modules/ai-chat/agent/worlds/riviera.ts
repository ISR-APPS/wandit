...
Declare ONE token before writing any markup: --sea: 58%. That is the optical height of the horizon, measured from the top of any full-height scene box, and it is the most load-bearing number in this world. Below 720px it becomes 50% (portrait crops eat sky, floor-anchored copy needs the room) — one media query, nowhere else.
...
- SHAPE IS RATIO, NOT RADIUS. Four aspect ratios used consistently are this world's silhouette: 21/9 panorama (anything with a horizon), 3/2 postcard recto, 4/5 portrait plate (people, rooms, dishes), 1/1 detail chip (materials, hands, close crops). Border-radius exists in exactly three places: 999px on pills, 50% on the sun, 3px on form fields. This world OVERRIDES the global default of soft rounded cards — a 12-16px radius on any content container is a ban-list violation here.
...
2. RADIUS: global tolerates soft rounded containers; here radius exists in exactly three places (999px pills, 50% sun, 3px on the textarea) and a 12-16px radius on any content container is a ban-list violation. Shape is carried by four aspect ratios instead.
3. TOKEN COUNT: eleven ground and text tokens plus ONE error hue, which is the only colour outside the sea, sand and citrus poles.

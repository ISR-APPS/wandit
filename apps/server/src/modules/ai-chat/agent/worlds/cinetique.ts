...
- IMAGERY: what the generative surfaces actually draw — the shader field and every Canvas2D system are illustrations of the verb, not decoration.
...
- A warm- or cool-shifted near-black field, one warm off-white, one dim companion tone, and ONE hyper-saturated accent. Four tokens, no more, plus a per-item accent injected as inline custom properties.
...
- SECOND PALETTE LAYER — the chromatic narrative: each showcase item carries inline custom properties for its own accent and a matching glow at 10-12% alpha, cascading into FIVE sub-elements — the full-bleed radial glow at 90% 70% at 80% 20% (opacity .5), the ghost numeral's 1px stroke at opacity .14, the index marker, the metric, and the media frame's corner ticks. Choose 4 hues that walk around the wheel from the brief's family (amber, violet, teal, ice-blue register). The page's color temperature therefore CHANGES AS YOU SCROLL while the base three-token system stays fixed. That is a narrative, not a theme.
...
- THE NAV IS NOT A BAR. position:fixed; inset:0 0 auto 0; z-index:100; flex with space-between; padding 1.1rem var(--pad); mix-blend-mode:difference. Zero background, zero blur, zero border, zero pill, no scrolled state, no hide-on-scroll. It inverts against the shader, against an accent slab, against everything. Links are mono .7rem uppercase at .12em with a 1px accent underline that scales X from 0 to 1 over .35s cubic-bezier(.65,0,.35,1), the transform-origin FLIPPING from right at rest to left on hover so it wipes in from one side and out the other. The wordmark is display type with a superscript accent mark.
...
## Motion identity (three.js + GSAP 3 + ScrollTrigger via CDN; native scroll — NO smooth-scroll library)
...
THE GENERATIVE HERO SURFACE (WebGL — preferred; the world still works without it via hero form 2)
- Orthographic camera (-1,1,1,-1,0,1) plus a 2x2 plane as a fullscreen quad with a shader material, depthTest and depthWrite false.
- Fragment program: a full simplex-noise implementation, then fBm over 5 octaves with lacunarity 2.02 and gain 0.52, then TWO-STAGE DOMAIN WARPING — q = fbm(p); r = fbm(p + q scaled 0.85/0.6 + a constant offset); v = fbm(p*1.6 + r scaled 1.1/0.75 + time*0.55). Ridges via pow(max(1.0 - abs(v), 0.0), 10.0). Then the move that matters: a LARGE-SCALE COVERAGE MASK, smoothstep(0.0, 0.85, fbm(p*0.55 + offset) + 0.35), which keeps MOST OF THE FRAME BLACK — that is what stops it from reading as a generic gradient mesh. Color from an iridescent cosine palette, multiplied by ridge and cover and about 1.7; a second thinner ridge added at power 14 and 0.10; the accent mixed in only at the ridge peaks via smoothstep(0.88, 1.0, ridge*cover) * 0.45; a vignette at smoothstep(1.45, 0.35, length(uv)); and a black lift of about +0.031/0.031/0.039 so nothing is crushed to pure black. Time scale: u_time * 0.04 — slow.
- INTERACTION: pointer normalized to -1..1 and lerped into the mouse uniform at 0.045 per frame, offsetting the noise domain by 0.24. Heavy inertia: the field drifts toward the cursor rather than tracking it. The hero is a surface, not a still.
- FALLBACK CONTRACT — three designed tiers, all mandatory: (1) if the webgl context throws or the library is undefined, return early and leave the animated CSS gradient fallback visible; only add the body class that hides the fallback AFTER the renderer actually constructs. (2) Under reduced motion, set the time uniform to a chosen constant (8.0), render exactly ONE frame, re-render on resize, and never start the animation loop — a composed frozen still, not a blank box. (3) The CSS fallback itself animates. A missing shader must never leave a dead rectangle.

...
FIXED TOKEN MAP — NOIR→--background · LUMEN→--foreground · ACCENT→--primary · NOIR→--primary-foreground · NOIR-2→--secondary · CHALK→--accent · lumen at 40%→--muted · lumen at 14%→--border · zero→--radius · DISPLAY→--font-heading · BODY→--font-body. These are the only CSS color/type/radius variables: every pole below is a prose role, literal register hexes belong only in these :root values, and every sibling, pool or alpha step is a color-mix(in srgb, var(--foreground) N%, transparent), substituting the mapped fixed token for that pole, or a color-mix between two mapped fixed-token references. Every unmapped role below MUST reuse the nearest mapped token or such a mix; it never gets another custom property or literal. Non-color mechanics may keep their own custom properties. Any rgba(POLE, a) notation below is design shorthand for that color-mix, never emitted CSS.
TEN tonal roles drawn from exactly THREE palette families — the noir/lumen neutrals, the accent metal and the white-cube pair — all routed through the fixed tokens. Validation is a mix of those poles, never a fourth family.
...
- CHALK plus INK (the white cube only): two scoped tones for exactly one section — var(--accent) supplies the gallery white that is not white, and the fixed --foreground override takes the near-black ink role.
- CARNELIAN is not another hue: a muted, contrast-safe color-mix between var(--primary) and var(--foreground), appearing in exactly two places, error text and the error field's underline.
...
And their counterweight, THE CONTACT SHADOW: a separate element, width 34 percent of the stage, height 1.6rem, radial-gradient(ellipse at 50% 50%, color-mix(in srgb, var(--background) 85%, transparent), transparent 70%), blur(8px), at the object's base. Light alone makes the object float; light plus contact shadow makes it stand.
...
1. LA VITRINE CENTRALE (default; the signature; jewelry, watches, perfume, gadgets, art). Single column, centred, min-height calc(100svh - 3.4rem), display grid, place-items center, padding-block clamp(3rem,7vh,5.5rem), overflow clip. Top to bottom: the collection line in accent micro-caps at .66rem/.32em; the hand-broken h1; THE STAGE; the cartel; the price in its claw setting; one primary CTA plus one quiet link; the facts rail.
   ...
   - z4 THE OBJECT: 2-4 absolutely stacked img elements at inset 0, width and height 100%, object-fit CONTAIN, filter drop-shadow(0 26px 34px color-mix(in srgb, var(--background) 62%, transparent)), all at opacity 0 except the first. Explicit width/height attributes; fetchpriority high on frame one.
...
- RADIUS IS ZERO. Buttons and CTAs consume var(--radius) directly (:root sets --radius: 0); every frame, plate, input, select, table, badge, plinth, rail and bar uses min(var(--radius), 0px). These var(--radius)-derived declarations are the single sanctioned corner mechanism; a pill exception is not permitted.
...
- SHADOWS: this world OVERRIDES the global shadow convention. Exactly two species, no third. (a) THE CONTACT SHADOW — a drawn blurred dark-pole ellipse element, never a box-shadow. (b) THE DROP-SHADOW FILTER on contained objects: drop-shadow(0 26px 34px color-mix(in srgb, var(--background) 62%, transparent)) on dark, drop-shadow(0 30px 40px color-mix(in srgb, var(--foreground) 22%, transparent)) in the white cube. ZERO box-shadows on cards, panels, buttons, inputs, the rail, the bar or the form plate. If you have written a box-shadow, you have left the world.
...
6. THE ROOM INVERSION — a ScrollTrigger on the white cube, start "top 60%", end "bottom 40%", toggling a class on the html element via onEnter/onLeave/onEnterBack/onLeaveBack. That class reassigns the applicable fixed tokens (--background, --foreground, --secondary, --muted and --border), with pool, plate and rail colours expressed directly as mixes of them; no room-specific color properties. Each transitions over .60s, and the class rewrites the theme-color meta. NOT two stacked sections cross-fading — a single fixed-token swap never breaks scroll height and reads as the lights changing in a room.
...
- THE STICKY ORDER BAR (la cimaise mobile) — minimal by design: silhouette, price, Acquérir. Fixed, inset-inline 0, bottom 0, z-index 95, height 4.4rem plus env(safe-area-inset-bottom) as padding-bottom, background rgba(noir,.86), backdrop-filter blur(18px), border-top 1px rgba(lumen,.14), radius min(var(--radius), 0px), NO shadow, grid auto 1fr auto at gap .9rem and padding-inline clamp(1rem,4vw,1.4rem).
...
1. Global: mid-radius rounded containers. HERE: var(--radius) is 0 and every CTA consumes it directly; every non-action box caps it at 0.

...
**BYO:** create an external-domain row → create the Cloudflare `www` custom hostname → best-effort **prepare the domain's Cloudflare zone** in our account (`findZoneByName` → adopt, else `createZone`; needs `CLOUDFLARE_ACCOUNT_ID` on the API server) → persist `dns.records` = `www` CNAME + ownership TXT **+ one `NS` record per zone nameserver (purpose `nameserver`, name `@`)** together with `dns.zoneId` / `zoneNameServers` / `zoneStatus` / `zoneCreated?` / `zoneDelegated: true` (a zone failure is logged, stored as `dns.apexError`, and the attach stays www-only exactly as before) → return the records → dispatch `domain-configure` with a global domain+nonce key → before every verification probe the task runs the **external** `ApexZoneStep` composition (see below) → durable verification resumes until active or returns external-pending. BYO needs neither registrar credentials nor payment.

The user is shown both options at once and picks one:
...
- **Option B — add DNS records only (unchanged behaviour):** the `www` CNAME + ownership TXT at the registrar; the bare domain must be redirected to `https://www.{domain}` at the registrar. Nothing about this path changed. Only rows attached with the kill switch off (or before this change, without a zone) stay on this option alone; a row whose attach-time zone preparation failed gets the nameservers from the next `domain-configure` pass, which finds-or-creates the zone and merges the `NS` records into `dns.records`.

Everything zone/apex related for external rows is best-effort: any failure is logged, stored in `dns.apexError`, and the www path continues exactly as today. Diagnostics resolve the `NS` records like any other row; while a user is on option B they read "missing"/"mismatch" for the NS rows and vice-versa — the option titles make that expected. Rows attached before this change (no zone) keep today's UI and are not backfilled (follow-up). A zone that no longer exists (deleted out of band, purged by Cloudflare while pending) is **withdrawn** as soon as a pass or a manual verify notices it (`getZoneStatus` → `null` on 404): the `NS` records leave `dns.records`, every zone key and `apexConfigured` are cleared, and `dns.apexError` says `Cloudflare zone {id} no longer exists`; a `configuring` row gets a fresh zone from its next configure pass, an `active` row stays on option B (no pass fills a zone recreated for it).
...
| `POST /api/v1/domains/:id/verify` | Recheck BYO certificate/configuration state; for an external row with a zone, withdraw the nameservers when that zone no longer exists |
...
| `CLOUDFLARE_ACCOUNT_ID` | Purchase (apex zone step), BYO configuration (external apex zone step), backfill — **and the Nest API deployment** (external attach prepares the zone) | Account that owns the per-domain zones (`POST /zones`). Not asserted at preflight: without it the apex step / the attach records `dns.apexError` and the domain stays www-only. |
| `DOMAINS_APEX_ZONE_ENABLED` | Purchase and BYO configuration (apex zone step), backfill, Nest API (external attach) | Kill switch, default `true`. `false` = no new zone: registrar URL forwarding for a purchased apex and www-only records for an external attach, exactly the previous behavior; an external row that already exposed its zone (`dns.zoneId`) is still finished. |
...
- The only keys added for the apex zone step are the optional `CLOUDFLARE_ACCOUNT_ID` and the `DOMAINS_APEX_ZONE_ENABLED` kill switch in `packages/env/src/server.ts`; both are now read by the API server as well (external attach).

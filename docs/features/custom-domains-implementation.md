...
**Prototype initial :** 2026-07-04 → 2026-07-05 · **Architecture actuelle mise à jour :** 2026-08-17 (Name.com + Stripe + Trigger.dev v4 + zone Cloudflare pour l'apex, achats et domaines externes)
...
- `ApexZoneStep` (best-effort, ne lève jamais; désactivable par `DOMAINS_APEX_ZONE_ENABLED=false` — « pas de nouvelle zone » : une ligne externe qui porte déjà `dns.zoneId` (nameservers déjà montrés au propriétaire) est quand même terminée; options `{ enabled, fallbackOrigin, sources }` — toute ligne dont la `source` n'est pas listée est rendue telle quelle) : zone Cloudflare dans notre compte (adoptée par nom si elle existe; une zone persistée qui n'existe plus est retirée — `getZoneStatus` → `null` sur 404 — puis recréée), **d'abord** suppression des A/AAAA/CNAME en conflit à `www` (erreur Cloudflare 81053) puis CNAME DNS-only `www` + TXT de propriété du hostname `www` dans la zone, **ensuite** hostname Cloudflare de l'apex (adopté par nom), suppression des conflits à `@` puis CNAME DNS-only apex + TXT de propriété apex — un échec du hostname apex ne prive donc jamais de `www` un propriétaire qui a déjà délégué —, marqueur `dns.zoneDelegated` puis `setNameservers` Name.com (achat seulement), activation check, marqueur `dns.apexConfigured` / erreur `dns.apexError`; seuls les enregistrements `NS` (purpose `nameserver`, nom `@`) sont fusionnés dans `dns.records` (le CNAME apex et les TXT vivent dans la zone); écrit ses clés `dns` par fusion jsonb (`DomainsRepository.mergeDnsIfStatus`, jamais de remplacement), rejoué avant chaque sonde du `DomainConfigurationRunner` (configuration tant que le marqueur manque, puis sondage de la zone : `pending` → activation check, `active` → un PATCH de revalidation du hostname apex), puis seulement par `domains:backfill-apex` une fois un domaine acheté `active`; l'activation du domaine n'attend jamais la zone ni le hostname apex. Compositions : `createDomainPurchaseRuntime` et le backfill câblent `sources: ["purchased"]` avec le registrar Name.com; `createDomainConfigurationRuntime` (`domain-configure`) câble `sources: ["external"]` avec un registrar dont `setNameservers` lève (`External domains delegate nameservers manually`) — jamais appelé pour une ligne externe. Pour une ligne externe, `configure()` diffère exactement en ceci : l'écriture de la zone persiste aussi les `NS` dans `dns.records` + `zoneDelegated: true` (le propriétaire peut déléguer à tout moment); import unique du DNS public actuel du domaine dans la zone (`scanDnsRecords`, marqueurs `dns.zoneScanned` / `dns.zoneScanRecordsAdded`) puis passage de tous les A/AAAA/CNAME importés en DNS-only (`disableProxyOnAllRecords`, exécuté même si le scan a échoué : un timeout côté client — le scan a son propre budget de 60 s — n'empêche pas Cloudflare de terminer l'import) avant l'écriture de nos enregistrements — un échec est journalisé et retenté à une passe suivante (ou à une passe `verify` tant que la zone est `pending`) sans interrompre la passe; aucun appel registrar; aucune écriture isolée de `zoneDelegated`. `verify()` est identique pour les deux sources; une zone disparue y est retirée de la ligne (`NS` hors de `dns.records`, clés de zone et `apexConfigured` effacées, `dns.apexError` = `Cloudflare zone {id} no longer exists`) et la passe de configuration suivante en crée une nouvelle;
...
BYO crée une ligne externe et le hostname Cloudflare `www`, puis prépare la zone Cloudflare du domaine dans notre compte en best-effort (`findZoneByName` → adoption, sinon `createZone`; `CustomerZoneService` est injecté dans `DomainsService`, `CLOUDFLARE_ACCOUNT_ID` est donc nécessaire sur le serveur API) et persiste en une seule écriture `dns.records` = CNAME `www` + TXT de propriété **+ un `NS` par nameserver de la zone** avec `zoneId` / `zoneNameServers` / `zoneStatus` / `zoneCreated?` / `zoneDelegated: true`; si la zone échoue (dont `CLOUDFLARE_ACCOUNT_ID` absent), l'attache reste www-only exactement comme avant avec `dns.apexError`. En rollback (handoff Trigger refusé), seule une zone créée par cet appel est supprimée, jamais une zone adoptée. Puis `domain-configure` est déclenché avec une clé globale domaine+nonce et exécute la composition externe d'`ApexZoneStep` avant chaque sonde. Il ne requiert ni Name.com ni Stripe. Deux options sont proposées en même temps à l'utilisateur : **A — nameservers Wandit** (recommandé, `example.com` et `www.example.com` fonctionnent; l'import du DNS existant préserve mail et sous-domaines, avec les mises en garde « vérifiez vos autres services » et « désactivez DNSSEC avant de basculer »); **B — enregistrements DNS seulement** (comportement inchangé : `www` seul, redirection de l'apex chez le registrar). Seules les lignes attachées avec le kill switch désactivé (ou avant ce changement, sans zone) restent sur la seule option B; une ligne dont la préparation de zone a échoué à l'attache reçoit les nameservers de la passe `domain-configure` suivante. Le `verify` manuel d'une ligne externe qui porte une zone vérifie que la zone existe encore et, sinon, retire ses nameservers (une ligne `configuring` reçoit une nouvelle zone de la tâche relancée; une ligne `active` reste en option B). La zone d'une ligne externe n'est jamais supprimée par le nettoyage (`bestEffortDeleteCustomerZone` refuse sur `source === "external"`); le détachement la laisse en place comme aujourd'hui. Les lignes attachées avant ce changement (sans zone) gardent l'interface actuelle et ne sont pas backfillées (suite).
...
| `CLOUDFLARE_ACCOUNT_ID` | Achat (étape zone apex), configuration BYO (étape zone apex externe), backfill — **et le déploiement API Nest** (l'attache externe prépare la zone) | Compte propriétaire des zones par domaine (`POST /zones`). Non asserté au préflight : absent, l'étape apex / l'attache écrit `dns.apexError` et le domaine reste www-only. |
| `DOMAINS_APEX_ZONE_ENABLED` | Achat et configuration BYO (étape zone apex), backfill, API Nest (attache externe) | Kill switch, défaut `true`. `false` = pas de nouvelle zone : forwarding URL registrar pour un apex acheté et enregistrements www-only pour une attache externe, comportement antérieur exact; une ligne externe qui a déjà exposé sa zone (`dns.zoneId`) est quand même terminée. |
...
- `domain-configure` valide DB + les trois valeurs Cloudflare avant la première sonde/mutation KV et lit le kill switch apex + fallback origin (`assertDomainConfigurationConfiguration()` renvoie désormais `apexZoneEnabled` et `fallbackOrigin`; `domainApexZoneOptions()` est le lecteur partagé); une ligne externe n'exige ni Name.com ni Stripe.
...
- Seules `CLOUDFLARE_ACCOUNT_ID` (optionnelle) et le kill switch `DOMAINS_APEX_ZONE_ENABLED` ont été ajoutés à `packages/env/src/server.ts` pour l'étape zone apex; les deux sont désormais aussi lus par le serveur API (attache externe).
...
Port `DomainProvider` : `checkAvailability`, `register`, `renew`, `setDnsRecords` (A/AAAA/CNAME/NS/TXT), `setNameservers` (`POST /core/v1/domains/{domain}:setNameservers`), `setUrlForwarding` (repli apex), `getAuthCode`, `setTransferLock`, `getDomainInfo`. Adapters Cloudflare : `CustomHostnameService` (hostname www canonicalisé, hostname apex nom nu, recherche par nom, statut, PATCH de revalidation, suppression) et `CustomerZoneService` (zones par domaine dans notre compte : recherche par nom, création avec `CLOUDFLARE_ACCOUNT_ID`, statut (`null` si la zone n'existe plus), activation check, upsert d'enregistrement DNS-only, import du DNS public `scanDnsRecords` (`POST /zones/{id}/dns_records/scan`, timeout propre de 60 s), `disableProxyOnAllRecords`, `deleteDnsRecords` (A/AAAA à un nom exact et tout CNAME différent de l'origine de repli), suppression).

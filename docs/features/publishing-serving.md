...
- Pixel snippets are split for page speed (`sites/domain/pixel-injector.ts`): the `fbq` / `ttq` stubs stay synchronous in `<head>`, but the vendor SDK fetch (`fbevents.js`, `events.js`) is deferred to `requestIdleCallback(fn, { timeout: 2500 })`, with a `setTimeout(fn, 1)` fallback. The stubs only QUEUE calls, so nothing is sent until the SDK arrives, and the delay is therefore BOUNDED: the fetch starts on the earliest of the `load` event, a 3 s timer, the page turning hidden, `pagehide`, or an explicit `window.wanditFlushPixels()`. The leads runtime calls that flush immediately after it queues a `Lead` (`leads/runtime/leads-runtime-script.ts`), so a conversion is never lost with the tab; the hidden and `pagehide` paths do the same for a bouncing visitor's `PageView`. A repeated call is a no-op (one-shot guard). Archived bytes keep their old snippet: a site gets the deferral on its next fresh publish, not on rollback.

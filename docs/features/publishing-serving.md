...
- Pixel snippets are split for page speed (`sites/domain/pixel-injector.ts`): the `fbq` / `ttq` stubs stay synchronous in `<head>`, but the vendor SDK fetch (`fbevents.js`, `events.js`) is deferred to `requestIdleCallback(fn, { timeout: 2500 })`, with a `setTimeout(fn, 1)` fallback. The stubs only QUEUE calls, so nothing is sent until the SDK arrives, and the delay is therefore BOUNDED: the fetch starts on the earliest of the `load` event, a 3 s timer, the page turning hidden, `pagehide`, or an explicit `window.wanditFlushPixels()`. The leads runtime calls that flush immediately after it queues a `Lead` plus any purchase conversions (`leads/runtime/leads-runtime-script.ts`), so a conversion is never lost with the tab; the hidden and `pagehide` paths do the same for a bouncing visitor's `PageView`. A repeated call is a no-op (one-shot guard). Rollback restamps recognized pixel blocks from current settings; an unrecognized marker carrier stays untouched.
...
**Acceptance criteria**
- Publish → page live on the sites domain, byte-identical to the draft version plus the deterministic publish passes: GSAP CDN inlining (`inlineKnownCdnScripts`), font-loading optimization (`optimizeFontLoading` — hoists the Google Fonts / Fontshare stylesheet links above the inline `<style>`, adds the two Google preconnects and `display=swap` when they are missing), image-markup normalization (`optimizeImageMarkup` — one elected LCP image with `fetchpriority="high"` and no `loading`, every other image `loading="lazy"` + `decoding="async"`, above-the-fold chrome left alone; it never writes `width`/`height`), responsive images (`emitResponsiveImages` — a `srcset`/`sizes="100vw"` built only from `.w{480,960,1600}.webp` renditions verified present in R2, plus one `<link rel="preload" as="image">` for the elected image; publish-only, because the editor's swap path strips `srcset` and the probe is network work a draft must not pay for), pixel injection when configured, the leads runtime, and the Wandit badge. All three injection passes replace recognized canonical blocks from current project state; if an unrecognized marker carrier survives, that pass returns its input untouched.

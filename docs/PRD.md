...
6. **Custom domains — buy in-app or bring your own** — sell domains at a markup through **Name.com CORE v1**, behind the existing `DomainProvider` port, plus Cloudflare for SaaS custom hostnames. Domain registration and renewal use direct checkout, never Wandit credits, and intentionally fail closed until `PaymentsModule` and a signature-verified payment webhook are integrated. Buying in-app unblocks Algerian users who can't pay international registrars (CIB/EDAHABIA). See `docs/features/custom-domains.md`.
